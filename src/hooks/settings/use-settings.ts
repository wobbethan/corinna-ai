...
export const useThemeMode = () => {
  const { setTheme, themes, forcedTheme, resolvedTheme, systemTheme, theme } =
    useTheme();
  return {
    theme,
    setTheme,
...
export const useSettings = (id: string) => {
  ...
  const onUpdateSettings = handleSubmit(async (values) => {
    ...
    if (values.domain) {
      ...
      if (domain) {
        toast({ title: "Success", description: domain.message });
      }
    }
    if (values.image[0]) {
...
export const useFilterQuestions = (id: string) => {
  ...
  const onAddFilterQuestionsHandler = handleSubmit(async (values) => {
    setLoading(true);
